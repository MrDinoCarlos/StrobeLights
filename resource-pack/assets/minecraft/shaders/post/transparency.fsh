...
int try_insert( sampler2D cSampler, sampler2D dSampler, vec2 coord, int ie ) {
    ...
    if (ie > 0) {
        ...
        // ItemDisplay geometry is composed after the main world. Without
        // lighting this layer, an opaque TRP beam pixel replaces a StrobeLights
        // contribution underneath it. Apply the same shared RGB light map to
        // the visible item layer; validated technical carriers returned above
        // remain byte-exact and invisible.
